...
describe('QookieModal', () => {
  ...
  it('the necessary category toggle is disabled', () => {
    expect(mountModal().findAll('.qookie-modal__toggle').at(0)!.attributes('disabled')).toBeDefined()
  })

  it('non-required toggles are not disabled', () => {
    const toggles = mountModal().findAll('.qookie-modal__toggle')
